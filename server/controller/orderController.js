...
const { CartItem, Order_items,Orders, Payment, Sessions, User, Products } = require('../models');

const { createError } = require('../error');

const getOrder = async (req, res, next) => {
    try {
        const orders = await Orders.find({ userId: req.user.id }).populate('orderSummary');
        res.statusCode = 200;
...
const getOrderItems = async (req, res, next) => {
    try {
        ...
        const orderDetails  = await Orders.findById(id).populate({path:'orderSummary',populate:{path:'productId'}});
