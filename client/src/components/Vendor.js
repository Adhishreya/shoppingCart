...
const Vendor = () => {
  ...
  useEffect(() => {
    userVendorProfile()
      .then((res) => setVendorDetails(res[0]))
...
const AddProducts = ({ approved, id }) => {
  ...
  const handleImage = (value, index) => {
    setImage(value);
    if (index >= images.length) setImages([...images, value]);
    else {
        ...
        <>
          ...
          <Form style={{ margin: "0rem" }} onSubmit={(e) => submitForm(e)}>
            ...
            <TextField
              required
              label="Product Image"
              onChange={(e) => handleImage(e.target.value, 0)}
            />
            ...
            {Array.from({ length: imageCount - 1 }).map((item, index) => (
              <ImageItem>
                <TextField
                  label="Product Image"
                  onChange={(e) => handleImage(e.target.value, index + 1)}
                />
